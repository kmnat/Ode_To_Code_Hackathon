...
export class GameDominoComponent implements AfterViewInit {
  ...
  submitPuzzle() {
    ...
    if (isCorrect) {
      ...
      if (this.current >= this.puzzles.length) {
        ...
        this.sendScoreToBackend(this.totalScore);
        setTimeout(() => {
  ...
  sendScoreToBackend(score: number) {
    ...
    fetch('http://localhost:5000/api/grads/update-score', {
      ...
      body: JSON.stringify({ score1: score }) // Using score1 for domino game
    })
    .then(async res => {
      if (!res.ok) {
    ...
    .then(data => {
      console.log('Domino game score (score1) updated successfully:', data);
    })
    .catch(err => {
      console.error('Failed to update domino game score:', err.message);
    });
  }
}
